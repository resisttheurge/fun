...
var data = [['pencils', 'drawing', 'bob ross', 'the public'], ['pencils', 'drawing', 'tony danza', 'cooksys'], ['pencils', 'drawing', 'bob ross', 'drc'], ['brushes', 'painting', 'darth vader', 'wdng'], ['brushes', 'painting', 'tony danza', 'svm'], ['brushes', 'burnishing', 'tony danza', 'amex'], ['brushes', 'burnishing', 'bob ross', 'aa.com'], ['markers', 'grading', 'tony danza', 'fedex'], ['marquees', 'sliding', 'bob ross', 'cdc'], ['glitter', 'bedazzling', 'tony danza', 'ssi'], ['crayons', 'coloring', 'darth vader', 'imc'], ['crayons', 'coloring', 'bob ross', 'post office']];
...
var resource = function resource(type, activity) {
  return { type: type, activity: activity };
};

var activity = function activity(type, admin) {
  return { type: type, admin: admin };
};

var admin = function admin(name, client) {
...
var full = function full(_ref) {
  var _ref2 = _slicedToArray(_ref, 4),
      ...
      activityType = _ref2[1],
      adminName = _ref2[2],
      clientName = _ref2[3];

  return resource(resourceType, activity(activityType, admin(adminName, client(clientName))));
};

var resources = data.map(full);

var base = function base() {
  return true;
};

var clientName = function clientName(name) {
  return function (resource) {
    return resource.activity.admin.client.name.match(name) !== null;
  };
};

var adminName = function adminName(name) {
  return function (resource) {
    return resource.activity.admin.name.match(name) !== null;
  };
};

var resourceType = function resourceType(name) {
  return function (resource) {
    return resource.type.match(name) !== null;
  };
};

var activityType = function activityType(name) {
  return function (resource) {
    return resource.activity.type.match(name) !== null;
  };
};

var or = function or(array) {
  return function (n) {
...
var and = function and(array) {
  return function (n) {
    return array.every(function (f) {
      return f(n);
    });
  };
};

var fromCategory = function fromCategory(searchFunction) {
  return function (searchTerms) {
    return searchTerms.length !== 0 ? or(searchTerms.map(searchFunction)) : base;
  };
};

var fromInput = function fromInput(input) {
  return and([fromCategory(clientName)(input.clientNames), fromCategory(adminName)(input.adminNames), fromCategory(resourceType)(input.resourceTypes), fromCategory(activityType)(input.activityTypes)]);
};

var solution = resources.filter(fromInput(input));

var print = function print(elem) {
  return console.log((0, _util.inspect)(elem, { depth: null }));
};
